...
static JSValue tjs_getgid(JSContext* ctx, JSValueConst this_val, int argc, JSValueConst* argv, int magic)
{
#if !defined(_WIN32)
    uid_t gid = magic ? getegid() : getgid();
...
static JSValue tjs_getuid(JSContext* ctx, JSValueConst this_val, int argc, JSValueConst* argv, int magic)
{
#if !defined(_WIN32)
    uid_t uid = magic ? geteuid() : getuid();
...
static JSValue tjs_print_all_handles(JSContext* ctx, JSValueConst this_val, int argc, JSValueConst* argv)
{
    if (argc > 0) {
        ...
        if (filename) {
            ...
            if (file) {
                uv_print_all_handles(TJS_GetLoop(ctx), file);
                fclose(file);
            }
    ...
    uv_print_all_handles(TJS_GetLoop(ctx), stdout);
    return JS_UNDEFINED;
}
...
{
    FILE* file = stdout;
    uv_print_active_handles(TJS_GetLoop(ctx), file);
    return JS_UNDEFINED;
}
...
static JSValue tjs_setgid(JSContext* ctx, JSValueConst this_val, int argc, JSValueConst* argv, int magic)
{
#if !defined(_WIN32)
    int32_t gid = 0;
    if (argc < 1 || (!JS_IsUndefined(argv[0]) && JS_ToInt32(ctx, &gid, argv[0]))) {
        return JS_UNDEFINED;
    }
...
static JSValue tjs_setuid(JSContext* ctx, JSValueConst this_val, int argc, JSValueConst* argv, int magic)
{
#if !defined(_WIN32)
    int32_t uid = 0;
    if (argc < 1 || (!JS_IsUndefined(argv[0]) && JS_ToInt32(ctx, &uid, argv[0]))) {
        return JS_UNDEFINED;
    }
...
static const JSCFunctionListEntry tjs_os_funcs[] = {
    ...
    TJS_CFUNC_MAGIC_DEF("getegid", 0, tjs_getgid, 1),
    TJS_CFUNC_MAGIC_DEF("geteuid", 0, tjs_getuid, 1),
    TJS_CFUNC_MAGIC_DEF("getgid", 0, tjs_getgid, 0),
    TJS_CFUNC_MAGIC_DEF("getuid", 0, tjs_getuid, 0),
    TJS_CFUNC_MAGIC_DEF("setegid", 1, tjs_setgid, 1),
    TJS_CFUNC_MAGIC_DEF("seteuid", 1, tjs_setuid, 1),
    TJS_CFUNC_MAGIC_DEF("setgid", 1, tjs_setgid, 0),
    TJS_CFUNC_MAGIC_DEF("setuid", 1, tjs_setuid, 0)
};
